...
typedef struct LTTO_IR_STORAGE {
	...
	LTTO_IR_STORAGE& operator= (const LTTO_IR_SIGNATURE& from);		//Used for converting an LTTO_IR_SIGNATURE to an LTTO_IR_STORAGE
	bool convertTo(LTTO_IR_SIGNATURE *output);						//Used for converting an LTTO_IR_STORAGE to an LTTO_IR_SIGNATURE.
} LTTO_IR_STORAGE;
